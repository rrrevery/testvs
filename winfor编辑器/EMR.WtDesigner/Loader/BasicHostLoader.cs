namespace EMR.WinformDesigner
{
	...
	public class BasicHostLoader: BasicDesignerLoader
	{
        ...
        public BasicHostLoader(byte[] by)
        {
            if (by == null)
            {
		...
		protected override void PerformLoad(IDesignerSerializationManager designerSerializationManager)
		{
			...
			else
			{
                ...
                baseClassName = ReadFile(b, errors, out xmlDocument);
			}
        ...
        public string SaveToDataBase(bool forceFilePrompt)
        {
            string cleanup;
            try
            {
                ...
                // the XML.
                //
                 cleanup = sw.ToString().Replace("<DOCUMENT_ELEMENT>", "");
