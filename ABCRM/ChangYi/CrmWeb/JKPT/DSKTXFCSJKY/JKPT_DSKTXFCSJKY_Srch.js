...
var vCaption = "单收款台消费次数监控月"
function InitGrid() {
    vColumnNames = ['月份', '月份', '收款台号码', '消费会员数量', '交易次数', '最大交易次数', '前5平均交易次数', '前10平均交易次数', '前20平均交易次数', 'mdid', ];
    vColumnModel = [             
               { name: 'iYEARMONTH', hidden: true, },
...
$(document).ready(function () {
    ...
    $("#list").datagrid({
        onDblClickRow: function (rowIndex, rowData) {
            MakeNewTab("CrmWeb/JKPT/XFCSJK/JKPT_XFCSJK_Srch.aspx?DZLX=5&rq=" + rowData.iYEARMONTH + "&mdid=" + rowData.iMDID + "&max_xfcs=" + rowData.iMAX_XFCS + "&sktno=" + $("#TB_SKTNO").val(), "单收款台消费次数日监控", vPageMsgID_XFCSJK);
        }
    });
...
function IsValidSearch() {
    if ($("#TB_RQ1").val() == "") {
        ShowMessage("请输入开始年月", 4);
        return false;
    }

    if ($("#TB_RQ2").val() == "") {
        ShowMessage("请输入结束年月", 4);
        return false;
    }
    if ($("#TB_RQ1").val() > $("#TB_RQ2").val()) {
        ShowMessage("开始年月不能大于结束年月", 4);
        return false;
    }
    if ($("#TB_SKTNO").val() == "") {
        ShowMessage("请输入收款台号");
...
function MakeSearchCondition() {
    ...
    MakeSrchCondition(arrayObj, "TB_RQ1", "iYEARMONTH", ">=", false);
    MakeSrchCondition(arrayObj, "TB_RQ2", "iYEARMONTH", "<=", false);
...
function DrawChart() {
    ...
    for (i = 0; i < lst.length ; i++) {
        var rowData = lst[i];
        xAxisArray.push(rowData.iYEARMONTH);
