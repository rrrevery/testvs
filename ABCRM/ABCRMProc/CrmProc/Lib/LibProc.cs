...
namespace BF.CrmProc
{
    public static class LibProc
    {
        ...
        {
            GlobalVariables.SYSInfo.bTest = GetWebConfig("Test", "false") == "true";
            GlobalVariables.SYSInfo.bUsePlatform = GetWebConfig("UsePlatform", "false") == "true";
            GlobalVariables.SYSInfo.iWriteCardPort = Convert.ToInt32(GetWebConfig("WriteCardPort", "22345"));
            GlobalVariables.SYSInfo.iWriteCardType = Convert.ToInt32(GetWebConfig("WriteCardType", "3"));
        ...
        public static bool LoginProc(out string msg, out LoginData login, string user, string pass, string str1)
        {
            ...
            else
            {
                ...
                if (query.IsEmpty)
                {
                    ...
                    {
                        login.iRYID = GlobalVariables.SYSInfo.iTestID;
                        login.sRYDM = user;
                        login.sRYMC = "测试用户";
                    }
                    else
                ...
                if (type.GetMethod(func) != null)
                {
                    ...
                    if (funcparam.Length == 3 && funcparam[0].Name == "msg" && funcparam[1].Name == "query" && funcparam[2].ParameterType.Name == "CRMLIBASHX")
                    {
                        ...
                        if ((outdata == null || outdata == "") && msg != "")
                            outdata = JsonConvert.SerializeObject(new ErrorMsg(msg));
                    }
                }
                if (!found)
                    switch (func)
        ...
        {
            return (int)Math.Floor((DateTime.Now - TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1))).TotalSeconds);
        }
        public static string MD5_ANSI(string s)
        {
        ...
        public static bool CheckSign(out string msg, string json, int ts, string sign)
        {
            msg = string.Empty;
            string s = json + "84982277" + ts;
            s = MD5_UTF8(s);
            ...
            else if (Math.Abs(LibProc.GetUnixTimestamp() - ts) > 120)
            {
                msg = "时间戳错误";
                return false;
            }
        ...
        {
            if (GlobalVariables.SYSInfo.bUsePlatform && token != "")
            {
