...
{
    public class MZKGL_YXQYC_Proc : DJLR_ZX_CLass
    {
        ...
        {
            CrmLibProc.DeleteDataTables(query, out msg, "MZK_YXQBDJL;MZK_YXQBDJLITEM;", "CZJPJ_JLBH", iJLBH);
        }
        ...
        public override void SaveDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            ...
            if (iJLBH != 0)
            {
                ...
                iJLBH = SeqGenerator.GetSeq("MZK_YXQBDJL");
            query.SQL.Text = "insert into MZK_YXQBDJL(CZJPJ_JLBH,HYKTYPE,ZY,XYXQ,DJSJ,DJR,DJRMC,CZDD)";
            ...
            {
                query.SQL.Text = "insert into MZK_YXQBDJLITEM(CZJPJ_JLBH,HYID,YYXQ,HYKNO)";
        ...
        {
            msg = string.Empty;
            ExecTable(query, "MZK_YXQBDJL", serverTime, "CZJPJ_JLBH");
            foreach (HYKGL_YXQYCItem one in itemTable)
            {
                query.SQL.Text = "update MZKXX set YXQ=:YXQ";
                if (iGGZT != 0)
                {
        ...
        public override List<Object> SearchDataQuery(CyQuery query, DateTime serverTime)
        {
            ...
            query.SQL.Text = "select W.*,D.HYKNAME,M.MDMC,B.BGDDMC,(select count(HYKNO) from MZK_YXQBDJLITEM I where I.CZJPJ_JLBH=W.CZJPJ_JLBH) KSL";
            query.SQL.Add(" from MZK_YXQBDJL W,HYKDEF D,HYK_BGDD B,MDDY M"); //
            query.SQL.Add(" where W.HYKTYPE=D.HYKTYPE");
            query.SQL.Add(" and W.CZDD=B.BGDDDM");
            query.SQL.Add(" and B.MDID=M.MDID(+)");
            if (sHYK_NO != "")
            {
                query.SQL.Add(" and exists(select 1 from MZK_YXQBDJLITEM I where I.CZJPJ_JLBH=W.CZJPJ_JLBH and  I.HYKNO='" + sHYK_NO + "')");
            }
            SetSearchQuery(query, lst);
            if (lst.Count == 1)
            {
                query.SQL.Text = " select I.*,D.HYKNAME from MZK_YXQBDJLITEM I,MZKXX H,HYKDEF D";
                query.SQL.Add(" WHERE I.HYID=H.HYID AND H.HYKTYPE=D.HYKTYPE(+) AND I.CZJPJ_JLBH=" + iJLBH);
                query.Open();
                while (!query.Eof)
                {
                    HYKGL_YXQYCItem item = new HYKGL_YXQYCItem();
                    ((MZKGL_YXQYC_Proc)lst[0]).itemTable.Add(item);
        ...
        {
            MZKGL_YXQYC_Proc obj = new MZKGL_YXQYC_Proc();
