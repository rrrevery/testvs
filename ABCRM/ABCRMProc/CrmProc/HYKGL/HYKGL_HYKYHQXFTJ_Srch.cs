...
{
  public   class HYKGL_HYKYHQXFTJ:YHQDEF
    {
      ...
      public override List<Object> SearchDataQuery(CyQuery query, DateTime serverTime)
      {
          ...
          if (iSEARCHMODE == 0)
          {
              ...
              query.SQL.Text = " select B.RQ, B.HYKTYPE,D.HYKNAME,B.YHQID,F.YHQMC,B.MDID,Y.MDMC,SUM(B.XFJE) XFJE,sum(B.TZJE) TZJE";
              ...
              MakeSrchCondition(query, " group by B.RQ, B.HYKTYPE,D.HYKNAME,B.YHQID,F.YHQMC,B.MDID,Y.MDMC");
          }
          if (iSEARCHMODE == 1)
          {
              CondDict.Add("dRQ", "B.RQ");
              query.SQL.Text = "  select B.RQ, B.HYKTYPE,D.HYKNAME,B.YHQID,F.YHQMC,B.MDID,Y.MDMC,B.SKTNO, SUM(B.XFJE) XFJE ,sum(B.TZJE) TZJE";
              query.SQL.Add("  from HYK_YHQ_XFRBB B,HYKDEF D,MDDY Y,YHQDEF F ");
              query.SQL.Add("   where B.HYKTYPE=D.HYKTYPE  and B.MDID=Y.MDID and B.YHQID=F.YHQID  ");
              query.SQL.Add("   and B.MDID=" + iMDID);
              query.SQL.Add("   and B.HYKTYPE=" + iHYKTYPE);
              query.SQL.Add("   and B.YHQID=" + iYHQID);
              MakeSrchCondition(query, "  group by B.RQ, B.HYKTYPE,D.HYKNAME,B.YHQID,F.YHQMC,B.MDID,Y.MDMC,B.SKTNO");
          }
          if (iSEARCHMODE == 2)
          {
      ...
      {
          HYKGL_HYKYHQXFTJ item = new HYKGL_HYKYHQXFTJ();
