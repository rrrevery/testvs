...
{
    public class HYXF_FQDYD : YHQCXD
    {
        public int iXFLJFQFS = 0;
        public int iBJ_HTFQ = 0;
        public int iLX = 0;

        public override void DeleteDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            CrmLibProc.DeleteDataTables(query, out msg, "HYKFQDYD;HYKFQDYD_FD;HYKFQDYD_GZSD;HYKFQDYD_GZITEM;HYKFQDYD_SP;", "JLBH", iJLBH);
        }
        HYXF_FQDYD()
        {
            sMainTable = "HYKFQDYD";
        ...
        public override void SaveDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            ...
            {
                iJLBH = SeqGenerator.GetSeq("HYKFQDYD");
            }

            query.SQL.Text = "insert into HYKFQDYD(JLBH,SHDM,SHBMID,SHBMDM,YHQID,CXID,YXCLBJ,RQ1,RQ2,DJR,DJRMC,DJSJ,STATUS,LX,DJLX,XFLJFQFS)";
            query.SQL.Add(" values(:JLBH,:SHDM,:SHBMID,:SHBMDM,:YHQID,:CXID,:YXCLBJ,:RQ1,:RQ2,:DJR,:DJRMC,:DJSJ,0,:LX,:DJLX,:XFLJFQFS)");
            ...
            query.ParamByName("XFLJFQFS").AsInteger = iXFLJFQFS;
            query.ExecSQL();
            foreach (CXD_FD one in itemFD)
            {
                query.SQL.Text = "insert into HYKFQDYD_FD(JLBH,INX,SD)";
            ...
            {
                query.SQL.Text = "insert into HYKFQDYD_GZSD(JLBH,INX,GZBH,CLFS_BM,CLFS_SPFL,CLFS_SPSB,CLFS_HT,BJ_CJ,CLFS_SP,CLFS_HYZ,FQGZID,CLFS_ZFFS,CLFS_BANK,CLFS_KH)";
                query.SQL.Add(" values(:JLBH,:INX,:GZBH,:CLFS_BM,:CLFS_SPFL,:CLFS_SPSB,:CLFS_HT,:BJ_CJ,:CLFS_SP,:CLFS_HYZ,:FQGZID,:CLFS_ZFFS,:CLFS_BANK,:CLFS_KH)");
                query.ParamByName("JLBH").AsInteger = iJLBH;
                query.ParamByName("INX").AsInteger = one.iINX;
                ...
                query.ParamByName("CLFS_ZFFS").AsInteger = one.iCLFS_ZFFS;
                query.ParamByName("CLFS_BANK").AsInteger = one.iCLFS_BANK;
                query.ParamByName("CLFS_KH").AsInteger = one.iCLFS_KH;
                query.ParamByName("FQGZID").AsInteger = one.iGZID;
                // query.ParamByName("CLDX").AsString = one.sCLDX;
                query.ExecSQL();
            }
            foreach (CXD_GZITEM one in itemGZITEM)
            {
                query.SQL.Text = "insert into HYKFQDYD_GZITEM(JLBH,INX,GZBH,SJLX,SJNR,XH)";
            ...
            //{
            //    query.SQL.Text = "insert into HYKFQDYD_KLX(JLBH,HYKTYPE)";
        ...
        public override List<Object> SearchDataQuery(CyQuery query, DateTime serverTime)
        {
            ...
            query.SQL.Text = "select A.*,B.BMMC,S.SHMC,Y.YHQMC,C.CXZT from HYKFQDYD A,SHBM B,SHDY S,YHQDEF Y,CXHDDEF C";
            query.SQL.Add(" where A.SHBMID=B.SHBMID and A.SHDM=S.SHDM and A.YHQID=Y.YHQID and A.CXID=C.CXID");
            SetSearchQuery(query, lst);
            if (lst.Count == 1)
            {
                query.SQL.Text = "select * from HYKFQDYD_FD where JLBH=" + iJLBH;
                query.Open();
                while (!query.Eof)
                {
                    ...
                    ((HYXF_FQDYD)lst[0]).itemFD.Add(item_fd);
                    query.Next();
                }
                query.SQL.Text = "select G.*,Z.YHQFFGZMC from HYKFQDYD_GZSD G,YHQFFGZ Z where G.FQGZID=Z.YHQFFGZID and JLBH=" + iJLBH;
                query.Open();
                while (!query.Eof)
                {
                    ...
                    item_gzsd.iGZID = query.FieldByName("FQGZID").AsInteger;
                    item_gzsd.sGZMC = query.FieldByName("YHQFFGZMC").AsString;
                    item_gzsd.sCLDX = GetCLDX(query);
                    item_gzsd.iXH = ((HYXF_FQDYD)lst[0]).itemGZSD.Count(item => item.iINX == item_gzsd.iINX) + 1;
                    ((HYXF_FQDYD)lst[0]).itemGZSD.Add(item_gzsd);
                    query.Next();
                }
                query.SQL.Text = "select * from  HYKFQDYD_GZITEM where JLBH=" + iJLBH;
                query.Open();
                while (!query.Eof)
                {
                    ...
                    ((HYXF_FQDYD)lst[0]).itemGZITEM.Add(item_cxd);
                    query.Next();
        ...
        public override object SetSearchData(CyQuery query)
        {
            ...
            obj.iXFLJFQFS = query.FieldByName("XFLJFQFS").AsInteger;
            return obj;
        }
    }
}
