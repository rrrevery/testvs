...
{
    public class HYKGL_HYKHS_Proc : HYK_DJLR_CLass
    {
        public int iTKSL = 0;
        public double fJE = 0;
        ...
        public override void SaveDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            ...
            if (iJLBH != 0)
            {
                ...
                iJLBH = SeqGenerator.GetSeq("HYK_TK");
            query.SQL.Text = "insert into HYK_TK(JLBH,HYKTYPE,ZY,TKSL,JE,TKJE,TKFS,DJR,DJRMC,DJSJ,MDID,BGR,BGRMC,BGDDDM,LYR,LYRMC,FXDWID)";
            ...
            {
                query.SQL.Text = "insert into HYK_TK_ITEM(JLBH,HYID,HYK_NO,CZKYE_OLD,YHQYE_OLD,JE,ZKL,TKJE,KFJE,QCJE,INX,BJ_CHILD)";
        ...
        {
            msg = string.Empty;
            ExecTable(query, "HYK_TK", serverTime);
            query.Close();
            query.SQL.Text = "insert into HYKCARD(CZKHM,CDNR,HYKTYPE,QCYE,YXTZJE,PDJE,BGDDDM,BGR,JKFS,SKJLBH,STATUS,YXQ,XKRQ)";
            query.SQL.Add(" select I.HYK_NO,H.CDNR,H.HYKTYPE,I.QCJE,0,0,H.YBGDD,:DJR,2,null,1,H.YXQ,:XKRQ");
            query.SQL.Add(" from HYK_TK_ITEM I,HYK_HYXX H where I.JLBH=:JLBH and I.HYID=H.HYID");
            query.ParamByName("JLBH").AsInteger = iJLBH;
            query.ParamByName("DJR").AsInteger = iLoginRYID;
            query.ParamByName("XKRQ").AsDateTime = serverTime;
            query.ExecSQL();
            query.SQL.Text = "update HYK_HYXX set OLD_HYKNO=HYK_NO,HYK_NO=LPAD(to_char(HYID),15,'0')||'X',CDNR=LPAD(to_char(HYID),15,'0')||'X',STATUS=-1";
            query.SQL.Add(" where HYID in(select HYID from HYK_TK_ITEM I where I.JLBH=:JLBH)");
            query.ParamByName("JLBH").AsInteger = iJLBH;
        ...
        public override List<Object> SearchDataQuery(CyQuery query, DateTime serverTime)
        {
            ...
            query.SQL.Add("     from HYK_TK W,HYK_BGDD B,HYKDEF D,FXDWDEF F");
            ...
            if (lst.Count == 1)
            {
                query.SQL.Text = "select I.* from HYK_TK_ITEM I,HYK_HYXX H where I.JLBH=" + iJLBH;
                query.SQL.Add(" and I.HYID=H.HYID");
                query.Open();
                while (!query.Eof)
                {
                    HYKGL_HYKHSItem obj = new HYKGL_HYKHSItem();
                    ((HYKGL_HYKHS_Proc)lst[0]).itemTable.Add(obj);
        ...
        {
            HYKGL_HYKHS_Proc obj = new HYKGL_HYKHS_Proc();
