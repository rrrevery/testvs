...
namespace BF.CrmProc
{
    public static class CrmLibProc
    {
        ...
        public static string Get_Userid(int personid, string name)
        {
            ...
            try
            {
                ...
                string sMGRPID = LibProc.GetWebConfig("MGRPID", "20");
                string sMENU = LibProc.GetWebConfig("MENUSET", "");//A:全部菜单
                iGrpID = Convert.ToInt32(sMGRPID);
                //模块
                if (sMENU != "A")
                    query.SQL.Text = "select * from " + sPubUser + ".MODULE_GROUP where SYSID=510 and MGRPID=" + iGrpID;
                else
                    query.SQL.Text = "select '测试模式' as MGRPMC from dual";
                query.Open();
                if (query.IsEmpty)
                    return str;
                str = " { id: \"a" + iGrpID + "\", pId: 0, name: \"" + query.FieldByName("MGRPMC").AsString + "\",title:\"\"},\r\n";
                //子模块
                query.Close();
                if (sMENU != "A")
                    query.SQL.Text = "select * from " + sPubUser + ".MODULE_DEF where MGRPID=" + iGrpID;
                else
                    query.SQL.Text = "select 1 as ID,'测试模块' as NAME from dual";
                query.Open();
                while (!query.Eof)
                {
                    str += " { id: \"b" + query.FieldByName("ID").AsInteger + "\", pId: \"a" + iGrpID + "\", name: \"" + query.FieldByName("NAME").AsString + "\",title:\"\"},\r\n";
                    query.Next();
                }
                //菜单组
                query.Close();
                if (sMENU != "A")
                {
                    query.SQL.Text = "select M.* from " + sPubUser + ".MODULE_DEF_MENU M," + sPubUser + ".MODULE_DEF D";
                    query.SQL.Add("where M.ID=D.ID and D.MGRPID=" + iGrpID + " and length(POS)=2");
                }
                else
                    query.SQL.Text = "select to_char(ID) POS,1 as ID,LIBNAME as CAPTION from " + sPubUser + ".SYSLIB where ID>=5160000 and ID<5170000 order by ID";
                query.Open();
                while (!query.Eof)
                {
                    str += " { id: \"c" + query.FieldByName("POS").AsString + "\", pId: \"b" + query.FieldByName("ID").AsInteger + "\", name: \"" + query.FieldByName("CAPTION").AsString + "\",title:\"\"},\r\n";
                    query.Next();
                }
                //末级菜单
                query.Close();
                if (sMENU != "A")
                {
                    query.SQL.Text = "select M.*,substr(POS,0,2) PPOS from " + sPubUser + ".MODULE_DEF_MENU M," + sPubUser + ".MODULE_DEF D";
                    query.SQL.Add("where M.ID=D.ID and D.MGRPID=" + iGrpID + " and length(POS)>2");
                }
                else
                {
                    query.SQL.Text = "select to_char(P.MSG_ID) POS,to_char(LIBID) as PPOS,CAPTION,W.URL from " + sPubUser + ".PROCMSGDEF P," + sPubUser + ".PROCMSGWEB W";
                    query.SQL.Add("where P.MSG_ID>=5160000 and P.MSG_ID<5170000 and P.MSG_ID=W.MSG_ID order by P.MSG_ID");
                }
                query.Open();
                while (!query.Eof)
                {
                    str += " { id: \"d" + query.FieldByName("POS").AsString + "\", pId: \"c" + query.FieldByName("PPOS").AsString + "\", name: \"" + query.FieldByName("CAPTION").AsString + "\",title:\"\",";
                    str += "value:\"" + query.FieldByName("URL").AsString + "\"},\r\n";
                    query.Next();
                }
                query.Close();
            }
            ...
            return "var " + name + " = [" + str.TrimEnd(',') + "];";

            /////////////////
            //string tp_1 = ""; string tp_x = "";
            //DataTable dta = new DataTable();
            ////dta = Get_Table_Menu_A(out msg, personid, "510", "99");
            //for (int i = 0; i <= dta.Rows.Count - 1; i++)
            //{
            //    tp_1 += " { id: " + (i + 1) + ", pId: 0, name: \"" + dta.Rows[i]["name"].ToString() + "\" ,title:\"1\"},";
            //    tp_1 += " \r\n";
            //    DataTable dtb = new DataTable();
            //    dtb = Get_Table_Menu_B(out msg, personid, "99");
            //    for (int j = 0; j <= dtb.Rows.Count - 1; j++)
            //    {
            //        tp_1 += " { id: " + (i + 1) + "" + (j + 1) + ", pId: " + (i + 1) + ", name: \"" + dtb.Rows[j]["name"].ToString() + "\" ,title:\"1\"},";
            //        tp_1 += " \r\n";
            //        DataTable dtc = new DataTable();
            //        dtc = Get_Table_Menu_C(out msg, personid, "99", "99");
            //        for (int k = 0; k <= dtc.Rows.Count - 1; k++)
            //        {
            //            tp_1 += " { id: " + (i + 1) + "" + (j + 1) + "" + (k + 1) + ", pId: " + (i + 1) + "" + (j + 1) + ", name: \"" + dtc.Rows[k]["name"].ToString() + "\" ,title:\"1\"},";
            //            tp_1 += " \r\n";
            //            DataTable dtd = new DataTable();
            //            dtd = Get_Table_Menu_D(out msg, personid, dtc.Rows[k]["ID"].ToString(), "99");
            //            for (int x = 0; x <= dtd.Rows.Count - 1; x++)
            //            {
            //                tp_1 += " { id: " + (i + 1) + "" + (j + 1) + "" + (k + 1) + "" + (x + 1) + ", pId: " + (i + 1) + "" + (j + 1) + "" + (k + 1) + ", name: \"" + dtd.Rows[x]["name"].ToString() + "\" ,title:\"1\",value:\"" + dtd.Rows[x]["URL"].ToString() + "\"},";
            //                tp_1 += " \r\n";
            //                if (i <= dta.Rows.Count - 1 && i >= 0 && j <= dtb.Rows.Count - 1 && j >= 0 && k <= dtc.Rows.Count - 1 && k >= 0 && x <= dtd.Rows.Count - 1 && x >= 0)
            //                {
            //                    tp_x += tp_1;
            //                    tp_1 = "";
            //                }
