...
{
    public class CRMREPORT_RFMFXZBMD_Proc : BASECRMClass
    {
        public int iLX = 0;
        ...
        {
            CrmLibProc.DeleteDataTables(query, out msg, "HYK_RFM_DEF_MD;", "JJRID", iJLBH);
            //CrmLib.CrmLibProc.DeleteDataBase(query, out msg, "delete from HYK_RFM_DEF_MD where MDID=" + iMDID + " and ND=" + iND + " and JB= " + iJB + "");
        }
        public override void SaveDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            ...
            if (sOldSHDM != "")
            {
                query.SQL.Text = "update HYK_RFM_DEF_MD set ND=:ND,MDID=:MDID,JB=:JB,XFJE_BEGIN=:XFJE_BEGIN,XFJE_END=:XFJE_END,LDCS_BEGIN=:LDCS_BEGIN,LDCS_END=:LDCS_END,LDPL_BEGIN=:LDPL_BEGIN,LDPL_END=:LDPL_END,LX=:LX";
            ...
            {
                query.SQL.Text = "insert into HYK_RFM_DEF(ND,MDID,JB,XFJE_BEGIN,XFJE_END,LDCS_BEGIN,LDCS_END,LDPL_BEGIN,LDPL_END,LX)";
                query.SQL.Add(" values(:ND,:MDID,:JB,:XFJE_BEGIN,:XFJE_END,:LDCS_BEGIN,:LDCS_END,:LDPL_BEGIN,:LDPL_END,:LX)");
                query.ParamByName("ND").AsInteger = iND;
                query.ParamByName("MDID").AsInteger =iMDID;
                query.ParamByName("JB").AsInteger = iJB;
                query.ParamByName("XFJE_BEGIN").AsInteger = iXFJE_BEGIN;
