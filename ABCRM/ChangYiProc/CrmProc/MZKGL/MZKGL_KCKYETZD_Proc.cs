...
{
    public class MZKGL_KCKYETZD_Proc : HYK_DJLR_CLass
    {
        ...
        {
            CrmLibProc.DeleteDataTables(query, out msg, "MZK_KCYETZD;MZK_KCYETZDITEM;", "JLBH", iJLBH);
        }
        ...
        public override void SaveDataQuery(out string msg, CyQuery query, DateTime serverTime)
        {
            ...
            if (iJLBH != 0)
            {
                ...
                iJLBH = SeqGenerator.GetSeq("MZK_KCYETZD");
            query.SQL.Text = "insert into MZK_KCYETZD(JLBH,HYKTYPE,BGDDDM,DJR,DJRMC,DJSJ)";
            ...
            {
                query.SQL.Text = "insert into MZK_KCYETZDITEM(JLBH,HM,QCYE_OLD,TZJE,QCYE_NEW)";
        ...
        {
            msg = string.Empty;
            ExecTable(query, "MZK_KCYETZD", serverTime);
            foreach (HYKGL_KCKYETZItem one in itemTable)
            {
                query.SQL.Text = "update MZKCARD set QCYE=:QCYE where CZKHM=:CZKHM";
                ...
                query.ExecSQL();
                CrmLibProc.SaveCardTrack(query, one.sCZKHM, 0, (int)BASECRMDefine.CZK_DKGZCLLX.CZK_CLLX_KCJETZ, iJLBH, iLoginRYID, sLoginRYMC, one.fQCYE_NEW - one.fQCYE_OLD);
            }
        ...
        public override List<Object> SearchDataQuery(CyQuery query, DateTime serverTime)
        {
            ...
            query.SQL.Add("     from MZK_KCYETZD W,HYK_BGDD B,HYKDEF D");
            query.SQL.Add("    where W.BGDDDM=B.BGDDDM and W.HYKTYPE=D.HYKTYPE");
            SetSearchQuery(query, lst);
            if (lst.Count == 1)
            {
                query.SQL.Text = "SELECT C.*,K.QCYE,K.YXQ,D.HYKTYPE,D.HYKNAME FROM MZK_KCYETZDITEM C,MZKCARD K,HYKDEF D where C.HM=K.CZKHM and K.HYKTYPE=D.HYKTYPE and C.JLBH=" + iJLBH;
                query.Open();
                while (!query.Eof)
                {
                    HYKGL_KCKYETZItem obj = new HYKGL_KCKYETZItem();
                    ((MZKGL_KCKYETZD_Proc)lst[0]).itemTable.Add(obj);
        ...
        {
            MZKGL_KCKYETZD_Proc obj = new MZKGL_KCKYETZD_Proc();
