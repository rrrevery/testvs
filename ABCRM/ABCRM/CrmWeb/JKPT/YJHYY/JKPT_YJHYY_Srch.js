...
var iSEARCHMODE = 0;
function InitGrid() {
	vColumnNames = ['会员', '会员卡号', "门店名称", '姓名', 'SEX', '性别', '年龄', '手机号码', '交易预警', '同款台预警', '同部门预警', '返利预警', ];
	vColumnModel = [
...
function YJMXGrid() {
	cols1= ['年月', 'iYJLX', '预警类型', 'iZBLX', '指标类型', '预警指标数', '消费门店', '卡类型', '交易次数', '超出比例(%)', '交易金额', '退货次数', '积分', '门店名称', '预警规则号', ];
	valCols1= [
		{ name: 'fYEARMONTH', width: 80, },
...
function MakeSearchCondition() {
	...
	MakeSrchCondition(arrayObj, "TB_YEARMONTH1", "fYEARMONTH", ">=", true);
	MakeSrchCondition(arrayObj, "TB_YEARMONTH2", "fYEARMONTH", "<=", true);
	return arrayObj;
}
