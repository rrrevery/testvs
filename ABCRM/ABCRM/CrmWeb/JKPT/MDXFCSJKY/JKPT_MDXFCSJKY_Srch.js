...
vCaption = "门店消费次数监控月";
function InitGrid() {
    vColumnNames = ['年月', '年月', 'MDID', '门店名称', '消费会员数量', '交易次数', '最大交易次数', '前5平均交易次数', '前10平均交易次数', '前20平均交易次数', '前30平均交易次数', '前50平均交易次数', ];
    vColumnModel = [
             //{ name: 'iJLBH', index: 'iJLBH', width: 80, hidden: true },//sortable默认为true width默认150
             { name: 'fYEARMONTH', hidden: true, },
             { name: 'sYEARMONTH', width: 80, },
...
$(document).ready(function () {
    ...
    $("#list").datagrid({
        onDblClickRow: function (rowIndex, rowData) {
            MakeNewTab("CrmWeb/JKPT/XFCSJK/JKPT_XFCSJK_Srch.aspx?DZLX=1&rq=" + rowData.fYEARMONTH + "&mdid=" + rowData.iMDID + "&max_xfcs=" + rowData.fMAX_XFCS, "消费次数月监控", vPageMsgID_XFCSJK);
        }
    });
...
function IsValidSearch() {
    ...
    if ($("#TB_YEARMONTH1").val() == "") {
        ShowMessage("请输入开始年月");
        return false;
    }

    if ($("#TB_YEARMONTH2").val() == "") {
        ShowMessage("请输入结束年月");
        return false;
    }
    if ($("#TB_YEARMONTH1").val() > $("#TB_YEARMONTH2").val()) {
        ShowMessage("开始年月不能大于结束年月");
        return false;
    }
    return true;
}
...
function MakeSearchCondition() {
    ...
    MakeSrchCondition(arrayObj, "TB_YEARMONTH1", "fYEARMONTH", ">=", false);
    MakeSrchCondition(arrayObj, "TB_YEARMONTH2", "fYEARMONTH", "<=", false);
    return arrayObj;
};
...
function DrawChart() {
    ...
    for (i = 0; i < lst.length ; i++) {
        var rowData = lst[i];
        xAxisArray.push(rowData.fYEARMONTH);
        ...
        plotOptions: {
            series: {
                events: {
                    click: function (e) {
                        if (this.name == "最大交易次数")
                            MakeNewTab("CrmWeb/JKPT/XFCSJK/JKPT_XFCSJK_Srch.aspx?DZLX=1&rq=" + e.point.category + "&mdid=" + $("#HF_MDID").val() + "&max_xfcs=" + e.point.y, "消费次数月监控", vPageMsgID_XFCSJK);
