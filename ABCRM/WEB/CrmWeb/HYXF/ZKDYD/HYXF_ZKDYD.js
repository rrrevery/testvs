...
function InitGrid() {
    vColumnNames = ["优先顺序", "规则号", "折扣率（%）", "处理标记", "处理对象", "分单编号", "iCLFS_BM", "iCLFS_SPFL", "iCLFS_SPSB", "iCLFS_HT", "iCLFS_SP", "iCLFS_HYZ", ];//"分值",
    vColumnModel = [
          { name: "iXH", },
          { name: "iGZBH", },
       // { name: "fFZ", width: 60, editable: true, editor: 'text' },
          { name: "fZKL", width: 60, editable: true, editor: 'text' },
...
function SaveData(Obj) {
    ...
    for (var i = 0; i < itemGZSD.length; i++) {
        if (itemGZSD[i].iBJ_CJ == "√")
            ...
            itemGZSD[i].iBJ_CJ = 0;
        itemGZSD[i].fZKL = parseFloat(itemGZSD[i].fZKL / 100);
    }
...
function ShowData(data) {
    ...
    for (var i = 0; i < Obj.itemGZSD.length; i++) {
        ...
        Obj.itemGZSD[i].fZKL = parseFloat(Obj.itemGZSD[i].fZKL * 100);
    }
