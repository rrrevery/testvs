...
function InitGrid() {
    vColumnNames = ["iHYID", "卡号", "原余额", "取款金额"];
    vColumnModel = [
            { name: "iHYID", hidden: true },
            { name: "sHYK_NO", width: 80, },
            { name: "fYJE", width: 60 },
            { name: "fQKJE", width: 60, editor: 'text', editrules: { required: true, number: true, minValue: 1 } },
    ];
};
...
function SaveData() {
    ...
    Obj.fQKJE = compute("list", "fQKJE");
    Obj.iQKSL = $("#list").datagrid("getRows").length;
    $("#LB_QKJE").text(Obj.fQKJE);
    $("#LB_QKSL").text(Obj.iQKSL);
    var lst = new Array();
...
function compute(lisName, colName) {
    ...
    for (var i = 0; i < rows.length; i++) {
        total = parseFloat(total) + parseFloat(rows[i]['' + colName + '']);
    }
    return total;
}
